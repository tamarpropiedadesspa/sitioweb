    ...
    <section id="clientes" className="py-16 bg-white text-slate-800 relative border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative max-w-6xl mx-auto px-8 sm:px-12">
          ...
          {/* Rejilla de Logos a Todo Color (Espaciado reducido y mayor tamaño) */}
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-4 items-center justify-items-center min-h-[160px]">
            {visibleLogos.map((client, idx) => (
              <a
                ...
                className="w-full flex items-center justify-center p-1 sm:p-2 transition-all duration-300 transform hover:scale-110 group cursor-pointer"
                title={client.name}
              >
                <img
                  src={client.logo}
                  alt={client.name}
                  className="h-24 sm:h-28 lg:h-32 w-full max-w-[220px] sm:max-w-[240px] object-contain transition-all duration-300 drop-shadow-sm"
                />
              </a>
            ))}
          </div>
